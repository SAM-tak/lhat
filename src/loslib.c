...
#if !defined(l_time_t)		// {
//
// type to represent time_t in Lhat
//
...
#if !defined(l_gmtime)		// {
//
// By default, Lhat uses gmtime/localtime, except when POSIX is available,
...
// By default, Lhat uses tmpnam except when POSIX is available, where
// it uses mkstemp.
// ===================================================================
//
...
static int getboolfield (lhat_State *L, const char *key) {
  int res;
  res = (lhat_getfield(L, -1, key) == LHAT_TNIL) ? -1 : lhat_toboolean(L, -1);
  lhat_pop(L, 1);
  return res;
}
...
static const char *checkoption (lhat_State *L, const char *conv,
  ...
  lhatL_argerror(L, 1,
    lhat_pushfstring(L, "invalid conversion specifier '%%%s'", conv));
