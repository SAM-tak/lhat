  ...
  for (; nargs--; arg++) {
    if (lhat_type(L, arg) == LHAT_TNUMBER) {
      ...
      int len = lhat_isinteger(L, arg)
                ...
                : fprintf(f, LHAT_NUMBER_FMT,
                             (LHATI_UACNUMBER)lhat_tonumber(L, arg));
      status = status && (len > 0);
    }
    else {
      size_t l;
...
static int f_seek (lhat_State *L) {
  ...
  lhatL_argcheck(L, (lhat_Integer)offset == p3, 3,
...
static void createstdfile (lhat_State *L, FILE *f, const char *k,
                           const char *fname) {
  LStream *p = newprefile(L);
  p->f = f;
  p->closef = &io_noclose;
