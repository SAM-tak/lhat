...
# elif defined(LHAT_USE_WINDOWS) && !defined(_CRTIMP_TYPEINFO) && defined(_MSC_VER) && (_MSC_VER >= 1400)
...
typedef lhatL_Stream LStream;


#define tolstream(L)	((LStream *)lhatL_checkudata(L, 1, LHAT_FILEHANDLE))

#define isclosed(p)	((p)->closef == NULL)


static int io_type(lhat_State *L)
{
    LStream *p;
    lhatL_checkany(L, 1);
    ...
    if(p == NULL)
        ...
        lhat_pushliteral(L, "file");
    return 1;
}


static int f_tostring(lhat_State *L)
{
    LStream *p = tolstream(L);
    if(isclosed(p))
        lhat_pushliteral(L, "file (closed)");
    else
        lhat_pushfstring(L, "file (%p)", p->f);
    return 1;
}


static FILE *tofile(lhat_State *L)
{
    LStream *p = tolstream(L);
    if(isclosed(p))
        lhatL_error(L, "attempt to use a closed file");
    lhat_assert(p->f);
    return p->f;
}
...
static int io_fclose(lhat_State *L)
{
    LStream *p = tolstream(L);
    int res = fclose(p->f);
    return lhatL_fileresult(L, (res == 0), NULL);
}


static LStream *newfile(lhat_State *L)
{
...
static void opencheck(lhat_State *L, const char *fname, const char *mode)
{
    LStream *p = newfile(L);
    p->f = fopen(fname, mode);
    if(p->f == NULL)
        lhatL_error(L, "cannot open file '%s' (%s)", fname, strerror(errno));
}


static int io_open(lhat_State *L)
{
...
static int io_popen(lhat_State *L)
{
    const char *filename = lhatL_checkstring(L, 1);
    const char *mode = lhatL_optstring(L, 2, "r");
    LStream *p = newprefile(L);
...
static int io_tmpfile(lhat_State *L)
{
    LStream *p = newfile(L);
    p->f = tmpfile();
    return (p->f == NULL) ? lhatL_fileresult(L, 0, NULL) : 1;
}


static FILE *getiofile(lhat_State *L, const char *findex)
{
    lhat_getfield(L, LHAT_REGISTRYINDEX, findex);
    LStream *p = (LStream *)lhat_touserdata(L, -1);
    if(isclosed(p))
        lhatL_error(L, "standard %s file is closed", findex + IOPREF_LEN);
    return p->f;
}


static int g_iofile(lhat_State *L, const char *f, const char *mode)
{
    if(!lhat_isnoneornil(L, 1)) {
        const char *filename = lhat_tostring(L, 1);
        if(filename)
...
static int io_input(lhat_State *L)
{
    return g_iofile(L, IO_INPUT, "r");
}


static int io_output(lhat_State *L)
{
...
// correct and to convert it to a L^ number
//
...
static int read_line(lhat_State *L, FILE *f, int chop)
{
    lhatL_Buffer b;
    int c = '\0';
    lhatL_buffinit(L, &b);
...
static int g_read(lhat_State *L, FILE *f, int first)
{
    int nargs = lhat_gettop(L) - 1;
    int success;
    int n;
    clearerr(f);
    if(nargs == 0) {  // no arguments?
        ...
        for(n = first; nargs-- && success; n++) {
            if(lhat_type(L, n) == LHAT_TNUMBER) {
                size_t l = (size_t)lhatL_checkinteger(L, n);
...
static int io_read(lhat_State *L)
{
    return g_read(L, getiofile(L, IO_INPUT), 1);
}


static int f_read(lhat_State *L)
{
...
{
    int nargs = lhat_gettop(L) - arg;
    int status = 1;
    for(; nargs--; arg++) {
        if(lhat_type(L, arg) == LHAT_TNUMBER) {
            // optimization: could be done exactly as for strings
            int len = lhat_isinteger(L, arg)
                ? fprintf(f, LHAT_INTEGER_FMT, (LHATI_UACINT)lhat_tointeger(L, arg))
                : fprintf(f, LHAT_NUMBER_FMT, (LHATI_UACNUMBER)lhat_tonumber(L, arg));
            status = status && (len > 0);
        }
        else {
            size_t l;
            const char *s = lhatL_checklstring(L, arg, &l);
...
static int io_write(lhat_State *L)
{
...
static int f_seek(lhat_State *L)
{
    static const int mode[] = { SEEK_SET, SEEK_CUR, SEEK_END };
    static const char *const modenames[] = { "set", "cur", "end", NULL };
    FILE *f = tofile(L);
    int op = lhatL_checkoption(L, 2, "cur", modenames);
    lhat_Integer p3 = lhatL_optinteger(L, 3, 0);
    l_seeknum offset = (l_seeknum)p3;
    lhatL_argcheck(L, (lhat_Integer)offset == p3, 3, "not an integer in proper range");
...
static int f_setvbuf(lhat_State *L)
{
    static const int mode[] = { _IONBF, _IOFBF, _IOLBF };
    static const char *const modenames[] = { "no", "full", "line", NULL };
    FILE *f = tofile(L);
    int op = lhatL_checkoption(L, 2, NULL, modenames);
    lhat_Integer sz = lhatL_optinteger(L, 3, LHATL_BUFFERSIZE);
...
static int io_flush(lhat_State *L)
{
    return lhatL_fileresult(L, fflush(getiofile(L, IO_OUTPUT)) == 0, NULL);
}


static int f_flush(lhat_State *L)
{
    return lhatL_fileresult(L, fflush(tofile(L)) == 0, NULL);
}
...
static void createstdfile(lhat_State *L, FILE *f, const char *k, const char *fname)
{
    LStream *p = newprefile(L);
    p->f = f;
    p->closef = &io_noclose;
