...
LHAT_API lhat_State *lhat_newcoroutine (lhat_State *L) {
  ...
  L1 = &cast(LX *, lhatM_newobject(L, LHAT_TCOROUTINE, sizeof(LX)))->l;
  ...
  memcpy(lhat_getextraspace(L1), lhat_getextraspace(g->maincoroutine),
         LHAT_EXTRASPACE);
...
LHAT_API lhat_State *lhat_newstate (lhat_Alloc f, void *ud) {
  ...
  for (i=0; i < LHAT_NUMTAGS; i++) g->mt[i] = NULL;
