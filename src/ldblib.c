//
// Interface from L^ to its debug API
...
//
// get (if 'get' is true) or set an upvalues from a closure
//
static int auxupvalue(lhat_State *L, int get)
{
    const char *name;
    int n = (int)lhatL_checkinteger(L, 2);  // upvalues index
...
//
// Check whether a given upvalues from a given closure exists and
// returns its index
//
static int checkupval(lhat_State *L, int argf, int argnup)
{
    int nup = (int)lhatL_checkinteger(L, argnup);  // upvalues index
    lhatL_checktype(L, argf, LHAT_TFUNCTION);  // closure
    lhatL_argcheck(L, (lhat_getupvalue(L, argf, nup) != NULL), argnup, "invalid upvalue index");
    return nup;
}
...
static int db_upvaluejoin(lhat_State *L)
{
    ...
    lhatL_argcheck(L, !lhat_iscfunction(L, 1), 1, "L^ function expected");
    lhatL_argcheck(L, !lhat_iscfunction(L, 3), 3, "L^ function expected");
...
static int db_sethook(lhat_State *L)
{
    ...
    lhat_rawset(L, -3);  // hooktable[L1] = new L^ hook
