...
// sizeof(integer) == 32.)
//
#if ((((LHAT_MAXINTEGER >> (NBM / 4)) >> (NBM / 4)) >> (NBM / 4)) \
	>> (NBM - (3 * (NBM / 4))))  >  0

#define l_intfitsf(i)  \
  (-((lhat_Integer)1 << NBM) <= (i) && (i) <= ((lhat_Integer)1 << NBM))
...
//
int lhatV_tonumber_ (const TValue *obj, lhat_Number *n) {
  TValue v;
  if (ttisinteger(obj)) {
    *n = cast_num(ivalue(obj));
    return 1;
            ...
            lhatO_str2num(svalue(obj), &v) == vslen(obj) + 1) {
    *n = nvalue(&v);  // convert result of 'lhatO_str2num' to a float
    return 1;
  }
  else
...
//
int lhatV_tointeger (const TValue *obj, lhat_Integer *p, int mode) {
  TValue v;
 again:
  if (ttisfloat(obj)) {
    lhat_Number n = fltvalue(obj);
    lhat_Number f = l_floor(n);
    if (n != f) {  // not an integral value?
      if (mode == 0) return 0;  // fails if mode demands integral value
      else if (mode > 1)  // needs ceil?
        f += 1;  // convert floor to ceil (remember: n != f)
    }
    return lhat_numbertointeger(f, p);
  }
    ...
    *p = ivalue(obj);
    return 1;
            ...
            lhatO_str2num(svalue(obj), &v) == vslen(obj) + 1) {
    obj = &v;
    goto again;  // convert result from 'lhatO_str2num' to an integer
  }
  return 0;  // conversion failed
}
...
// the number is too large, it is OK to set the limit as LHAT_MAXINTEGER,
// which means no limit.  If the number is too negative, the loop
// should not run, because any initial integer value is larger than the
// limit. So, it sets the limit to LHAT_MININTEGER. 'stopnow' corrects
// the extreme case when the initial value is LHAT_MININTEGER, in which
// case the LHAT_MININTEGER limit would still run the loop once.
//
static int forlimit (const TValue *obj, lhat_Integer *p, lhat_Integer step,
                     int *stopnow) {
  *stopnow = 0;  // usually, let loops run
  if (!lhatV_tointeger(obj, p, (step < 0 ? 2 : 1))) {  // not fit in integer?
    lhat_Number n;  // try to convert to float
    if (!tonumber(obj, &n)) // cannot convert to float?
      return 0;  // not a number
    if (lhati_numlt(0, n)) {  // if true, float is larger than max integer
      *p = LHAT_MAXINTEGER;
      if (step < 0) *stopnow = 1;
    }
...
//
void lhatV_finishget (lhat_State *L, const TValue *t, TValue *key, StkId val,
                      const TValue *slot) {
  ...
  for (loop = 0; loop < MAXTAGLOOP; loop++) {
    ...
    if (lhatV_fastget(L,t,key,slot,lhatH_get)) {  // fast track?
      setobj2s(L, val, slot);  // done
      return;
    }
    // else repeat (tail call 'lhatV_finishget')
  }
  lhatG_runerror(L, "'__index' chain too long; possible loop");
}
...
// to the entry 't[key]', or to 'lhatO_nilobject' if there is no such
// entry.  (The value at 'slot' must be nil, otherwise 'lhatV_fastset'
// would have done the job.)
//
void lhatV_finishset (lhat_State *L, const TValue *t, TValue *key,
  ...
  for (loop = 0; loop < MAXTAGLOOP; loop++) {
    ...
    if (lhatV_fastset(L, t, key, slot, lhatH_get, val))
      return;  // done
    // else loop
  }
  lhatG_runerror(L, "'__newindex' chain too long; possible loop");
}
...
// in false.
//
static int LTintfloat (lhat_Integer i, lhat_Number f) {
#if defined(l_intfitsf)
  if (!l_intfitsf(i)) {
    if (f >= -cast_num(LHAT_MININTEGER))  // -minint == maxint + 1
      return 1;  // f >= maxint + 1 > i
    else if (f > cast_num(LHAT_MININTEGER))  // minint < f <= maxint ?
      return (i < cast(lhat_Integer, f));  // compare them as integers
    else  // f <= minint <= i (or 'f' is NaN)  -->  not(i < f)
      return 0;
  }
#endif
  return lhati_numlt(cast_num(i), f);  // compare them as floats
}
...
//
static int LEintfloat (lhat_Integer i, lhat_Number f) {
#if defined(l_intfitsf)
  if (!l_intfitsf(i)) {
    if (f >= -cast_num(LHAT_MININTEGER))  // -minint == maxint + 1
      return 1;  // f >= maxint + 1 > i
...
#endif
  return lhati_numle(cast_num(i), f);  // compare them as floats
}
...
static int LTnum (const TValue *l, const TValue *r) {
  if (ttisinteger(l)) {
    ...
    lhat_Number lf = fltvalue(l);  // 'l' must be float
    if (ttisfloat(r))
      return lhati_numlt(lf, fltvalue(r));  // both are float
    else if (lhati_numisnan(lf))  // 'r' is int and 'l' is float
...
static int LEnum (const TValue *l, const TValue *r) {
  if (ttisinteger(l)) {
    ...
    lhat_Number lf = fltvalue(l);  // 'l' must be float
    if (ttisfloat(r))
      return lhati_numle(lf, fltvalue(r));  // both are float
    else if (lhati_numisnan(lf))  // 'r' is int and 'l' is float
      return 0;  //  NaN <= i is always false
    else  // without NaN, (l <= r)  <-->  not(r < l)
...
//
int lhatV_lessthan (lhat_State *L, const TValue *l, const TValue *r) {
  int res;
...
//
int lhatV_lessequal (lhat_State *L, const TValue *l, const TValue *r) {
  int res;
  if (ttisnumber(l) && ttisnumber(r))  // both operands are numbers?
    return LEnum(l, r);
  else if (ttisstring(l) && ttisstring(r))  // both are strings?
...
//
// Main operation for equality of Lhat values; return 't1 == t2'.
// L == NULL means raw equality (no metamethods)
//
int lhatV_equalobj (lhat_State *L, const TValue *t1, const TValue *t2) {
  const TValue *tm;
  if (ttype(t1) != ttype(t2)) {  // not the same variant?
    if (ttnov(t1) != ttnov(t2) || ttnov(t1) != LHAT_TNUMBER)
      return 0;  // only numbers can be equal with different variants
    else {  // two numbers with different variants
      lhat_Integer i1, i2;  // compare them as integers
      return (tointeger(t1, &i1) && tointeger(t2, &i2) && i1 == i2);
    }
  }
  // values have same type and same variant
  switch (ttype(t1)) {
    case LHAT_TNIL: return 1;
    case LHAT_TNUMINT: return (ivalue(t1) == ivalue(t2));
    case LHAT_TNUMFLT: return lhati_numeq(fltvalue(t1), fltvalue(t2));
    case LHAT_TBOOLEAN: return bvalue(t1) == bvalue(t2);  // true must be 1 !!
    case LHAT_TLIGHTUSERDATA: return pvalue(t1) == pvalue(t2);
    case LHAT_TLCF: return fvalue(t1) == fvalue(t2);
    case LHAT_TSHRSTR: return eqshrstr(tsvalue(t1), tsvalue(t2));
    case LHAT_TLNGSTR: return lhatS_eqlngstr(tsvalue(t1), tsvalue(t2));
    case LHAT_TUSERDATA: {
...
// macro used by 'lhatV_concat' to ensure that element at 'o' is a string
#define tostring(L,o)  \
	(ttisstring(o) || (cvt2str(o) && (lhatO_tostring(L, o), 1)))
...
//
void lhatV_concat (lhat_State *L, int total) {
  lhat_assert(total >= 2);
  do {
    StkId top = L->top;
    ...
    if (!(ttisstring(top-2) || cvt2str(top-2)) || !tostring(L, top-1))
      ...
      for (n = 1; n < total && tostring(L, top - n - 1); n++) {
        ...
        if (l >= (MAX_SIZE/sizeof(char)) - tl)
          lhatG_runerror(L, "string length overflow");
        tl += l;
      }
      if (tl <= LHATI_MAXSHORTLEN) {  // is result a short string?
        char buff[LHATI_MAXSHORTLEN];
        copy2buff(top, n, buff);  // copy strings to buffer
        ts = lhatS_newlstr(L, buff, tl);
      }
      else {  // long string; copy strings directly to final result
        ts = lhatS_createlngstrobj(L, tl);
...
//
void lhatV_objlen (lhat_State *L, StkId ra, const TValue *rb) {
  const TValue *tm;
  switch (ttype(rb)) {
    case LHAT_TTABLE: {
      Table *h = hvalue(rb);
...
//
lhat_Integer lhatV_div (lhat_State *L, lhat_Integer m, lhat_Integer n) {
  if (l_castS2U(n) + 1u <= 1u) {  // special cases: -1 or 0
    if (n == 0)
      lhatG_runerror(L, "attempt to divide by zero");
    return intop(-, 0, m);   // n==-1; avoid overflow with 0x80000...//-1
  }
  else {
    lhat_Integer q = m / n;  // perform C division
...
// about lhatV_div.)
//
lhat_Integer lhatV_mod (lhat_State *L, lhat_Integer m, lhat_Integer n) {
  if (l_castS2U(n) + 1u <= 1u) {  // special cases: -1 or 0
    if (n == 0)
      lhatG_runerror(L, "attempt to perform 'n%%0'");
...
#define NBITS	cast_int(sizeof(lhat_Integer) * CHAR_BIT)

//
// Shift left operation. (Shift right just negates 'y'.)
//
lhat_Integer lhatV_shiftl (lhat_Integer x, lhat_Integer y) {
  if (y < 0) {  // shift right?
    if (y <= -NBITS) return 0;
...
//
static LClosure *getcached (Proto *p, Upvalue **encup, StkId base) {
  LClosure *c = p->cache;
...
//
// create a new Lhat closure, push it in the stack, and initialize
// its upvalues. Note that the closure is not cached if prototype is
// already black (which means that 'cache' was already cleared by the
// GC).
//
static void pushclosure (lhat_State *L, Proto *p, Upvalue **encup, StkId base,
                         StkId ra) {
  int nup = p->sizeupvalues;
  Upvaldesc *uv = p->upvalues;
  int i;
  ...
  for (i = 0; i < nup; i++) {  // fill in its upvalues
    if (uv[i].instack)  // upvalues refers to local variable?
      ...
      ncl->upvalues[i] = encup[uv[i].idx];
    ncl->upvalues[i]->refcount++;
...
//
void lhatV_finishOp (lhat_State *L) {
  CallInfo *ci = L->ci;
  ...
  switch (op) {  // finish its execution
    ...
    case OP_LE: case OP_LT: case OP_EQ: {
      ...
      if (ci->callstatus & CIST_LEQ) {  // "<=" using "<" instead?
        lhat_assert(op == OP_LE);
        ci->callstatus ^= CIST_LEQ;  // clear mark
        res = !res;  // negate result
      }
      lhat_assert(GET_OPCODE(*ci->u.l.savedpc) == OP_JMP);
    ...
    case OP_CONCAT: {
      ...
      if (total > 1) {  // are there elements to concat?
        ...
        lhatV_concat(L, total);  // concat them (may yield again)
      }
    ...
    case OP_TFORCALL: {
      lhat_assert(GET_OPCODE(*ci->u.l.savedpc) == OP_TFORLOOP);
...
//
// {==================================================================
// Function 'lhatV_execute': main interpreter loop
// ===================================================================
//


//
// some macros for common tasks in 'lhatV_execute'
//
...
#define dojump(ci,i,e) \
  { int a = GETARG_A(i); \
    if (a != 0) lhatF_close(L, ci->u.l.base + a - 1); \
...
#define checkGC(L,c)  \
	{ lhatC_condGC(L, L->top = (c),  /* limit of live values */\
...
#define gettableProtected(L,t,k,v)  { const TValue *slot; \
  ...
  else Protect(lhatV_finishget(L,t,k,v,slot)); }


// same for 'lhatV_settable'
#define settableProtected(L,t,k,v) { const TValue *slot; \
  if (!lhatV_fastset(L,t,k,slot,lhatH_get,v)) \
    Protect(lhatV_finishset(L,t,k,v,slot)); }
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_CALL) {
        ...
        if (lhatD_precall(L, ra, nresults)) {  // C function?
          ...
          ci = L->ci;
          goto newframe;  // restart lhatV_execute over new Lhat function
        }
        vmbreak;
      }
      vmcase(OP_TAILCALL) {
