...
// sizeof(integer) == 32.)
//
# if ((((LHAT_MAXINTEGER >> (NBM / 4)) >> (NBM / 4)) >> (NBM / 4)) >> (NBM - (3 * (NBM / 4))))  >  0
#  define l_intfitsf(i)  \
...
//
static int forlimit(const TValue *obj, lhat_Integer *p, lhat_Integer step, int *stopnow)
{
...
//
void lhatV_finishget(lhat_State *L, const TValue *t, TValue *key, StkId val, const TValue *slot)
{
...
//
void lhatV_finishset(lhat_State *L, const TValue *t, TValue *key, StkId val, const TValue *slot)
{
    for(int loop = 0; loop < MAXTAGLOOP; loop++) { // counter to avoid infinite loops
...
//
// Main operation for equality of L^ values; return 't1 == t2'.
...
//
// create a new L^ closure, push it in the stack, and initialize
