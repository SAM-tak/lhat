...
//
// vm.c's build_environment makes the values and check.c's environment_type
// the types, and the two lists have to say the same thing. This is the one
// list; each side prepares a `<name>_value` or `<name>_type` local and
// expands it, so a member added here without both halves fails to compile,
// and one added to only one side no longer compiles at all.
//
