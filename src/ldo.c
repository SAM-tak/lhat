...
/*
** LHATI_THROW/LHATI_TRY define how Lhat does exception handling. By
** default, Lhat handles errors with exceptions when compiling as
...
** separated.
*/
static int moveresults (lhat_State *L, const TValue *firstResult, StkId res,
                                      int nres, int wanted) {
...
int lhatD_precall (lhat_State *L, StkId func, int nresults) {
  ...
  switch (ttype(func)) {
    ...
    case LHAT_TLCL: {  /* Lhat function: prepare its call */
      StkId base;
...
/*
** Call a function (C or Lhat). The function to be called is at *func.
...
void lhatD_call (lhat_State *L, StkId func, int nResults) {
  if (++L->nCcalls >= LHATI_MAXCCALLS)
    stackerror(L);
  if (!lhatD_precall(L, func, nResults))  /* is a Lhat function? */
...
static void finishCcall (lhat_State *L, int status) {
  ...
  /* finish 'lhat_callk'/'lhat_pcall'; CIST_YPCALL and 'errfunc' already
     handled */
...
static void resume (lhat_State *L, void *ud) {
  ...
  if (L->status == LHAT_OK) {  /* starting a coroutine? */
    if (!lhatD_precall(L, firstArg - 1, LHAT_MULTRET))  /* Lhat function? */
      lhatV_execute(L);  /* call it */
  }
    ...
    if (isLhat(ci))  /* yielded inside a hook? */
      lhatV_execute(L);  /* just continue running Lhat code */
    else {  /* 'common' yield */
...
LHAT_API int lhat_yieldk (lhat_State *L, int nresults, lhat_KContext ctx,
                        lhat_KFunction k) {
  CallInfo *ci = L->ci;
...
int lhatD_pcall (lhat_State *L, Pfunc func, void *u,
...
int lhatD_protectedparser (lhat_State *L, ZIO *z, const char *name,
                                        const char *mode) {
  struct SParser p;
  int status;
