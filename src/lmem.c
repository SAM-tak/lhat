...
void *lhatM_growaux_ (lhat_State *L, void *block, int *size, size_t size_elems,
...
l_noret lhatM_toobig (lhat_State *L) {
  lhatG_runerror(L, "memory allocation error: block too big");
}
...
void *lhatM_realloc_ (lhat_State *L, void *block, size_t osize, size_t nsize) {
  ...
  lhat_assert((nsize == 0) == (newblock == NULL));
