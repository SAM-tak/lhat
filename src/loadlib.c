...
** Dynamic library loader for Lhat
...
** when searching for a Lhat loader.
*/
...
#define DLMSG	"dynamic libraries not enabled; check your Lhat installation"
...
** variables that Lhat check to set its paths.
*/
...
** Set a path
*/
static void setpath (lhat_State *L, const char *fieldname,
                                   const char *envname,
                                   const char *dft) {
...
static void *checkclib (lhat_State *L, const char *path) {
  ...
  plib = lhat_touserdata(L, -1);  /* plib = CLIBS[path] */
...
static const char *pushnexttemplate (lhat_State *L, const char *path) {
  ...
  l = strchr(path, *LHAT_PATH_SEP);  /* find next separator */
...
static const char *searchpath (lhat_State *L, const char *name,
                                             const char *path,
                                             const char *sep,
                                             const char *dirsep) {
...
static int ll_searchpath (lhat_State *L) {
  const char *f = searchpath(L, lhatL_checkstring(L, 1),
...
static const char *findfile (lhat_State *L, const char *name,
                                           const char *pname,
                                           const char *dirsep) {
  const char *path;
  lhat_getfield(L, lhat_upvalueindex(1), pname);
  path = lhat_tostring(L, -1);
  if (path == NULL)
...
static int checkload (lhat_State *L, int stat, const char *filename) {
  if (stat) {  /* module loaded successfully? */
    ...
    return lhatL_error(L, "error loading module '%s' from file '%s':\n\t%s",
                          lhat_tostring(L, 1), filename, lhat_tostring(L, -1));
}
...
static int searcher_Lhat (lhat_State *L) {
  ...
  filename = findfile(L, name, "path", LHAT_LSUBSEP);
...
static int loadfunc (lhat_State *L, const char *filename, const char *modname) {
  ...
  mark = strchr(modname, *LHAT_IGMARK);
  if (mark) {
    int stat;
...
static int searcher_Croot (lhat_State *L) {
  ...
  filename = findfile(L, lhat_tostring(L, -1), "cpath", LHAT_CSUBSEP);
...
static void set_env (lhat_State *L) {
  ...
  if (lhat_getstack(L, 1, &ar) == 0 ||
    ...
    lhatL_error(L, "'module' not called from a Lhat function");
...
static void createsearcherstable (lhat_State *L) {
  ...
  for (i=0; searchers[i] != NULL; i++) {
    lhat_pushvalue(L, -2);  /* set 'package' as upvalue for all searchers */
...
LHATMOD_API int lhatopen_package (lhat_State *L) {
  ...
  lhat_pushglobaltable(L);
  lhat_pushvalue(L, -2);  /* set 'package' as upvalue for next lib */
