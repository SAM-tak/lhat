...
static int lhatB_setmetatable(lhat_State *L)
{
    ...
    lhatL_argcheck(L, t == LHAT_TNIL || t == LHAT_TTABLE, 2, "nil or table expected");
...
{
    int t = lhat_type(L, 1);
    lhatL_argcheck(L, t == LHAT_TTABLE || t == LHAT_TSTRING, 1, "table or string expected");
...
static int pairsmeta(lhat_State *L, const char *method, int iszero, lhat_CFunction iter)
{
    lhatL_checkany(L, 1);
