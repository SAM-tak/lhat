...
//
// This file uses only the official API of L^.
...
static void pushfuncname(lhat_State *L, lhat_Debug *ar)
{
    ...
    else if(*ar->what != 'C')  // for L^ functions, use <file:line>
...
{
    int en = errno;  // calls to L^ API may change this value
    if(stat) {
...
{
    const char *name = (def) ? lhatL_optstring(L, arg, def) : lhatL_checkstring(L, arg);
    for(int i = 0; lst[i]; i++)
...
// this extra space, L^ will generate the same 'stack overflow' error,
// but without 'msg'.)
//
...
LHATLIB_API const char *lhatL_optlstring(lhat_State *L, int arg, const char *def, size_t *len)
{
...
LHATLIB_API int lhatL_loadbufferx(lhat_State *L, const char *buff, size_t size, const char *name, const char *mode)
{
    LoadS ls;
    ls.s = buff;
    ls.size = size;
...
static const char *lhatL_findtable(lhat_State *L, int idx, const char *fname, int szhint)
{
    const char *e;
...
{
    lhat_writestringerror("PANIC: unprotected error in call to L^ API (%s)\n", lhat_tostring(L, -1));
    return 0;  // return to L^ to abort
}
...
LHATLIB_API void lhatL_checkversion_(lhat_State *L, lhat_Number ver, size_t sz)
{
    ...
    if(v != lhat_version(NULL))
        lhatL_error(L, "multiple L^ VMs detected");
    else if(*v != ver)
        lhatL_error(L, "version mismatch: app. needs %f, L^ core provides %f",
