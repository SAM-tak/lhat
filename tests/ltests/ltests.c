...
** Internal Module for Debugging of the Lhat Implementation
...
static int tpanic (lhat_State *L) {
  fprintf(stderr, "PANIC: unprotected error in call to Lhat API (%s)\n",
                   lhat_tostring(L, -1));
  return (exit(EXIT_FAILURE), 0);  /* do not return to Lhat */
}
...
static int upvalue (lhat_State *L) {
