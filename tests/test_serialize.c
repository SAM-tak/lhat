...
static const char *const SPECIMEN =
    ...
    "let^ Vec = t^{number^[2]}\n"
    "let^ Map = t^{[string^]:string^}\n"
    "let^ dict:Map = {key = \"value\"}\n"
    "if^ typeof^(dict).signature != \"t^{ [string^] : string^ }\" { return^ -20 }\n"
    "if^ {bad = 1} fits^ Map { return^ -21 }\n"
    "let^ lookup = f^ d:Map, k:string^ -> string^|nil^ { return^ d[k] }\n"
    "if^ lookup(dict, \"key\") != \"value\" { return^ -22 }\n"
    "let^ Pt = def^{\n"
    "    self^{ x := 0, y := 0 },\n"
