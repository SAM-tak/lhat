...
静的自動型付け(Static Auto Typing) = 漸進的型付け(Gradual Typing) + 双方向型推論 (Bidirectional Type Inference)。

> 双方向型推論 (Bidirectional Type Inference)
>
> 変数がどう宣言されたかだけでなく、変数の使用状況からもその型を推論する。
>
> 既に型が確定している関数 `f^x:number^;` の引数として型未確定の変数`a` を `f(a)` として渡したなら、
> この変数`a`の型は`number^`だろう、と推論するというもの。

ユーザーに型アノテーションをなるべく書かせない。
組み込み型/関数/手続き、ライブラリやホストアプリケーションからインポートされる型/関数/手続きが基礎の静的型情報を提供し、ユーザープログラムの型情報はすべて型推論から構築可能で型アノテーションの手書きは不要である、というようなものを目指している。
...
~~なので、++ += とかはない~~
複合代入はこの原則から除外。そもそも再代入とセットだから関数としての演算子としては扱わないとするだけでよい。
...
~~`$symbol` はグローバルへの参照~~ グローバルは、昨今の情勢からしてないほうが良いので、廃止。ファイルをまたいでの共有はモジュールを使う。
`$symbol` ファイルスコープ
`$$symbol` ファイルスコープの１個下のスコープ
`$^symbol` 現在のリテラルスコープの親
`$^^symbol` 現在のリテラルスコープの親の親
...
2)if/for/classなどの{}を要求する構文ではブロックであり、その文脈以外では{}はテーブルリテラル。制御文以外でブロックを定義したいときにdo^{ }
