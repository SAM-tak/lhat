...
//
// Two of the three questions never get that far: a module path and a
// require^'s string are read off the text, because neither has a tree worth
// asking (a half-written path resolves to nothing, and an unterminated
// string is one error token running to the end of the file).
...
// `text` is the whole document and `offset` the cursor.
