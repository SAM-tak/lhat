...
（`dap_normalize_path`、Windows は大小無視）して照合する。写像が知らない
ファイルにはブレークポイントが結ばれない。呼び出しはセッションのスレッドから
錠の下で来る——速く、スレッド安全に。

**正規化はリンクを踏み抜く。** 綴りの整形（`.` と `..`、相対の絶対化、
区切り）だけでは、シンボリックリンクやジャンクション越しに置いた
ブレークポイントが、その先の単位に結ばれない。POSIX は `realpath` が
最初からそこまでやっていた。Windows の `_fullpath` は**リパースポイントを
歩かない**ので、開いて訊く——`CreateFileA` して
`GetFinalPathNameByHandle`。昇格は要らない（リンクの指す先を*読む*のは特権
行為ではなく、*作る*方だけがそうである）し、desired access 0 で開くので
読み取り権限も要らない。返る `\?\` 接頭辞は剥がす——ここ以外のパスは
人が書く形をしているので、比較して等しくならなければ意味が無い。

**開けないパスは綴りの整形どまり**にする。エディタは既に動かされた
ファイルについて訊いてくることがある。

**正規化するのは照合の側だけで、報告の側ではない。** フレームが答える
`source` はプログラムが持っている綴り（写像があればエディタの綴り）のまま。
実体パスに直して返すと、書き手がリンク越しに開いているファイルとは別の
タブがもう一枚開く。同じファイルなのに二枚になる。

ハードリンク・bind マウント・同じボリュームの二重マウント・アーカイブ内の
単位は、これでも同一視できない。**写像がそのために在る。**
...
無し。D1 から D8 まで閉じた——経緯は改定履歴に。

次に来るとすれば、この章が扱ってこなかったもの: 逆行実行、データ
ブレークポイント（`setDataBreakpoints`）、ホストが用意する仮想の値
（`variablePresentationHint`）。いずれも今の形の不足ではなく別の機能で、
要ると分かった時点で番号を起票する。
...
- D6 閉鎖。写像なしの正規化がリンクを踏み抜く（§5.2）。Windows は
  `GetFinalPathNameByHandle`（昇格不要）、POSIX は元から `realpath`。
  正規化は照合の側だけで、フレームは貰った綴りのまま報告する。ハードリンクや
  bind マウントは届かないままで、そこは写像の領分。
