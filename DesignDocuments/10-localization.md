...
- **CLI** — `--language` で言語を選ぶ。カタログは `--messages FILE` か、
  実行ファイルの隣の `messages/<言語タグ>.msg` から読む。
  OS のロケールに既定で従うかは［未決 I1］
...
1. **ID と引数の構造化** — 英語の出力は今と一字一句同じにする。
   既存のテストと出力の比較で、退行が無いことを確かめられる
2. **英語をテンプレートにする** — 5.1 の3か所を穴に置き換える。
