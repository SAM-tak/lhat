...
これが唯一の言語側の変更で、8.2 が文の位置で拒む「裸の式」からもこの形だけは
除いてある — 点は既に報せてあり、節点こそが問いの対象だからである。
...
構文エラーの有無に関わらず走る。足したのは一点だけで、**末尾の `.` に引数の無い
メンバを残す**こと。型の路（`parse_type`）と修飾名（`parse_qualified_name`）は
変えない: 後者を変えると `module^ a.` が単位の名前を `"a."` にしてしまい、
