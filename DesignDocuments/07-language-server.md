...
size_t lhat_program_completion_modules(program, prefix, length, into, capacity);
```

- **モジュールもコアが答える。** 登録したのはプログラムなので、それを持つホストが
  一覧を別に抱える理由が無い。答えるのは接頭辞の次の**一区切りだけ**で、同じものは
  一度。`std.` の下は `io` と `math` であって `std.io` や `std.math.vector3` では
  ない——次に書かれるのは一区切りであり、その先はその区切りの点が打たれてから
  改めて出る
- **残るのは単位の一覧だけで、それはホストのもの。** どのファイルが在るかは
  ホストのファイル系が言うことである。コアは**どの問いか**と**書きかけがどこから
  始まるか**だけを答え、候補はホストが並べる
- `ask_text` は単位を要らない。木を訊く価値の無い二つ（書きかけのモジュール経路は
  何にも解決せず、閉じていない文字列は末尾まで走る一つの誤りトークン）を、
  検査を1回払う前に答えられる
- **`import^` の経路は語のすぐ後ろから始まってよい。** 01 の 2.3 でハットは名前の
  一部なので、カーソルから名前の字を遡ると `import^godot` は一続きに読める——
  これはこのリポジトリ自身の書き方である。語は綴りが決まっているので、遡った
  一続きがその語で始まっていれば経路はその直後から始まる
...
四つの問いのうち高いのはメンバだけである。書かれたメンバは一覧を歩くだけだが、
組込みのメンバは**検査器が知っている綴りを一つずつ訊く**もので
（`lhat_check_builtin_members`）、それが要求の9割を占める。ポップアップは打鍵ごとに
訊き直し、その間レシーバは変わらないので、同じ答えを何度も組み直すことになる——
エンジンのノードのように広いホスト型では1回が数ミリ秒になる。

**覚える場所はプログラムである。** 検査結果ではない——打鍵は単位を検査し直すので、
検査結果に置いた控えは、読まれるのと同じ頻度で捨てられる。

**鍵はレシーバの型のアドレスで、中身は一度も読まない。** プログラムの型は全て一つの
アリーナに在り（`lhat_check_unit` の共有 `arena`）、そのアリーナはプログラムが生きて
いる間 空にならない——検査し直しは古い型の**上にではなく隣に**作る。だからアドレスは
一つの型を指したまま変わらず、どの単位からも参照されなくなった型の控えは「間違った
答え」ではなく「誰も訊かない答え」になる。

**席は16、レシーバのアドレスで直に決まる。** 一件がまるごとの答え（216 メンバの型で
85 KB ほど）なので表は小さく、衝突は上書きで済ませる。検査し直しのたびに作り直される
型（`def^` は毎回そうなる）は一つの席で入れ替わり続け、同じアドレスで二度訊かれる
登録済みの型を押し出さない。

**捨てるのは登録が増えたときだけ。** レシーバの答えを変えるのはそれだけである——
ホスト型へのメンバ登録は、鍵にしているその型そのものに書き込む。05 の 8.7 の共有
モジュールが登録件数を見るのと同じ読み方で、3つの数を控えて比べる。
...
> **修正案はライブラリが作る。道具はそれを編集に変換するだけである。**

拒んだ段が、何が正しかったかを知っている。サーバで作り直せば 4 章と同じ理由で
スコープ規則の2つ目の実装になり、難しい場合にだけ食い違う。診断が自分の修正案を持ち、
`lhat_unit_diagnostic_fix`（`lhat/program.h`）がそれを答える。

- 修正案は **編集の並び** である。置き換える範囲と、そこに書く文字列（`LhatFixEdit`）。
  長さ 0 は挿入、空の文字列は削除
- **確度を2値で持つ。** 機械適用可（`LHAT_FIX_MACHINE`）は読まずに当ててよい物、
  要確認（`LHAT_FIX_SUGGESTED`）は何のつもりだったかの推測である。一括適用と、
  エディタが最初に出す1件（`isPreferred`）はこれで決まる
- **題名は人に見せる文** なので ID を持ち、訳せる（10 §4）。出どころは `fix`。
  題名は最初の編集が書く文字列を `{text}` として描く
- 言語サーバは `textDocument/codeAction` で `quickfix` として返す。
...
VS Code 拡張は両方の設定ファイルをワークスペース内で監視する。編集・作成・削除・
rename の通知を受けると読み直し、根の集合を作り直し、全体を検査し直す
（`lsp/worker.c`）。サーバーの再起動は要らない。
...
- **レシーバの答えを覚える（4.x改）。** メンバの補完はプログラムに16席の控えを持ち、
  レシーバの型のアドレスで直に引く。鍵にできるのは、プログラムの型が一つのアリーナに
  在り、それがプログラムの生涯 空にならないからである——検査し直しは古い型の隣に
  作るので、アドレスが別の型を指すことが無い。捨てるのは登録が増えたときだけで、
  それが答えを変える唯一のもの。216 メンバのホスト型で 665 µs → 75 µs（debug）、
  210 µs → 実質 0（release）

- **`import^` の綴りと、モジュールの持ち主（4.x改）。** Godot バインディングからの
  2件。`import^godot` は語と経路の間に何も無く、ハットが名前の一部（01 の 2.3）なので
  カーソルから名前の字を遡ると一続きに読める——語で始まっていれば経路はその直後から
  始まる、とした。そしてモジュールの一覧はコアに入れた（`lhat_program_completion_modules`）。
  登録したのはプログラムなので、ホストが一覧を別に抱える理由が無い。答えるのは
  接頭辞の次の一区切りだけで、同じものは一度
